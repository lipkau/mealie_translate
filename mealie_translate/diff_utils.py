...
class DiffFormatter:
    ...
    def _format_field_diff(
        ...
        if isinstance(original, list) and isinstance(translated, list):
            ...
            # Handle string fields (title, description, instructions)
            lines.extend(
    ...
    def _format_list_diff(
        ...
        """Format diff for list fields (ingredients)."""
        lines = []
        ...
        for i in range(max_items):
            ...
            if orig_item:
                # For ingredients, show the display text
                orig_text = getattr(orig_item, "display", str(orig_item))
                truncated = (
            ...
            if trans_item:
                trans_text = getattr(trans_item, "display", str(trans_item))
                truncated = (
